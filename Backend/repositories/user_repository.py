...
def save_user(student_id, username=None, display_name=None, line_user_id=None):
    ...
    try:
        cursor.execute("""
            INSERT INTO users (student_id, username, display_name, line_user_id, is_line_notify_active, last_login)
            VALUES (?, ?, ?, ?, ?, datetime('now', 'localtime'))
            ON CONFLICT(student_id) DO UPDATE SET
                username              = excluded.username,
                display_name          = excluded.display_name,
                line_user_id          = COALESCE(excluded.line_user_id, users.line_user_id),
                is_line_notify_active = CASE WHEN excluded.line_user_id IS NOT NULL THEN 1 ELSE users.is_line_notify_active END,
                last_login            = datetime('now', 'localtime')
        """, (student_id, username, display_name, line_user_id, 1 if line_user_id else 0))
        conn.commit()
        return True
